...
+
ISP가 고정 IP 주소와 기본 게이트웨이를 할당했다면 _x.x.x.x_ 를 고정 IP 주소로 바꾸고 _y.y.y.y_ 를 기본 게이트웨이의 IP 주소로 바꾸세요. ISP가 게이트웨이 주소 없이 고정 IP 주소만 제공한 경우, _y.y.y.y_ 를 `10.0.0.2/0` 으로 바꾸세요.
+
...
Line 17:::
이 줄은 게이트웨이에 기본 경로를 추가하므로 그대로 유지합니다. `HISADDR` 은 자동으로 16줄에 지정된 게이트웨이 주소로 바뀝니다. 이 줄은 16줄 뒤에 표시되는 것이 중요합니다.

man:ppp[8]가 수동으로 시작되는지 자동으로 시작되는지에 따라 다음 줄이 포함된 [.filename]#/etc/ppp/ppp.linkup# 을 생성해야 할 수도 있습니다. 이 파일은 `-auto` 모드에서 `ppp` 를 실행할 때 필요합니다. 이 파일은 연결이 설정된 후에 사용됩니다. 이 시점에서 IP 주소가 할당되고 이제 라우팅 테이블 항목을 추가할 수 있습니다. 이 파일을 만들 때 _provider_ 가 [.filename]#ppp.conf# 의 11번째 줄에 표시된 값과 일치하는지 확인하세요.
...
Line 13:::
이 줄은 PAP/CHAP 사용자 이름을 지정합니다. _MyUserName_ 에 올바른 값을 입력합니다.

Line 14:::
이 줄은 PAP/CHAP 비밀번호를 지정합니다. _MyPassword_ 에 올바른 값을 입력합니다. 다음과 같이 추가 줄을 추가할 수 있습니다:
+
[.programlisting]
....
16      accept PAP
...
`mgetty` 를 사용하면 모뎀과 활발하게 _대화_ 하므로 [.filename]#/etc/ttys# 에서 포트가 꺼져 있으면 모뎀이 전화를 받지 않는다는 이점이 있습니다.
...
수동으로 `ppp` 를 제어하여 인터넷에 연결하는 것은 빠르고 쉬우며, 연결을 디버깅하거나 ISP가 `ppp` 클라이언트 연결을 처리하는 방식에 대한 정보를 얻을 수 있는 좋은 방법입니다. 명령줄에서 PPP를 시작하겠습니다. 모든 예제에서는 PPP를 실행하는 컴퓨터의 호스트 이름으로 _example_ 을 사용한다는 점에 유의하세요. `PPP` 를 시작하려면:
...
여기에 기본 경로를 추가하는데, 현재 설정된 유일한 연결은 피어와의 연결이므로 외부와 대화하기 전에 이 작업을 수행해야 합니다. 기존 경로로 인해 연결이 실패하면 `add` 앞에 느낌표 `!` 를 넣으세요. 또는 실제 연결하기 전에 이 값을 설정하면 그에 따라 새 경로를 협상합니다.
...
여기서 _myusername_ 은 ISP에서 할당된 사용자 아이디로 대체해야 합니다.
...
여기서 _mypassword_ 는 ISP에서 할당받은 비밀번호로 대체해야 합니다.
...
여기서 _x.x.x.x_ 및 _y.y.y.y_ 는 ISP의 DNS 서버의 IP 주소로 대체해야 합니다.
...
최후의 수단으로 package:net/rr-pppoe[] 패키지 또는 포트를 설치해 볼 수 있습니다. 하지만 이렇게 하면 모뎀의 프로그래밍이 해제되어 쓸모없게 될 수 있으므로 신중하게 생각하세요. 모뎀과 함께 제공된 프로그램을 설치하기만 하면 됩니다. 그런 다음 프로그램에서 menu:System[] 메뉴에 액세스합니다. 프로필 이름이 나열되어 있어야 합니다. 보통 _ISP_ 입니다.
...
_xl1_ 을 이더넷 카드에 적합한 장치로 변경하는 것을 잊지 마세요.

_ISP_ 를 프로필로 변경하는 것을 잊지 마세요.
