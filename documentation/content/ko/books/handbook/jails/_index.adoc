---
description: 'jail은 여러 가지 방식으로 기존 chroot 환경의 개념을 개선합니다'
...
Jail은 자체적인 사용자 세트를 가지고 있으며 jail 내부에만 국한되는 `root` 계정 역시 가지고 있습니다. Jail의 `root` 계정은 jail 환경 외부에서 시스템 작업을 수행할 수 없습니다.
...
이러한 공격의 대부분은 호스트 환경에서 권한이 없는 사용자가 jail root에 액세스할 수 없도록 함으로써 완화할 수 있습니다. 기본적으로 jail에 대한 권한이 있는 신뢰할 수 없는 사용자에게 호스트 환경에 대한 액세스 권한을 부여해서는 안 됩니다.
====
...
<.> Jail의 위치를 선택하는 것이 가장 좋은 출발점입니다. Jail은 jail을 운영하는 호스트의 파일 시스템 내에서 물리적으로 위치하게 됩니다. 보통 _jailname_ 은 jail을 구분하는 호스트 네임이므로 [.filename]#/usr/jail/jailname# 은 jail의 이름으로 적절한 선택입니다. 보통 [.filename]#/usr/# 에는 충분한 공간이 있으며, "완전한" jail의 경우 기본적으로 FreeBSD 기본 시스템을 모두 복제합니다.
...
jail이 설치되면 man:jail[8] 유틸리티를 사용하여 jail을 시작할 수 있습니다. man:jail[8] 유틸리티는 <<jails-synopsis>> 에 설명된 네 가지 필수 인수를 받습니다. 특정 사용자의 자격증명을 사용하여 jail화 된 프로세스를 동작시키기 위해 다른 인수를 지정할 수도 있습니다. `_command_` 인수는 jail의 유형에 따라 달라집니다; _가상 시스템_ 의 경우, 실제 FreeBSD 시스템의 시작 시퀀스를 복제하기 때문에 [.filename]#/etc/rc# 은 좋은 선택입니다. _service_ jail의 경우, jail 내에서 실행될 서비스 또는 애플리케이션에 따라 다릅니다.
...
이러한 변수는 _호스트 시스템_ 의 시스템 관리자가 `root` 사용자에게 기본으로 부과하는 일부 제한을 추가하거나 제거하는 데 사용할 수 있습니다. 몇가지 제한은 제거할 수 없습니다. `root` 사용자는 man:jail[8] 내에서 파일 시스템을 마운트하거나 마운트를 해제할 수 없습니다. jail 내부의 `root` 는 man:devfs[8] 규칙 집합을 로드 또는 언로드하거나 방화벽 규칙을 설정하거나 커널의 `securelevel` 설정과 같이 커널 내 데이터를 수정해야 하는 기타 많은 관리 작업을 수행할 수 없습니다.
...
* Host 시스템에서 실행 중인 jail에 연결하여 jail 내부에서 명령을 실행하거나 jail 자체에서 관리 작업을 수행합니다. 이것은 특히 `root` 사용자가 jail을 깔끔하게 종료하고 싶을 때 유용합니다. 예를 들어, man:jexec[8] 유틸리티를 사용하여 jail 내부의 셸을 시작할 수도 있습니다:
+
[source, shell]
....
# jexec 1 tcsh
...
새로운 메이저 버전이나 마이너 버전으로 Jail을 업그레이드하려면 먼저 crossref:cutting-edge[freebsdupdate-upgrade,"메이저 및 마이너 버전 업그레이드 수행하기"]에 설명된 대로 host 시스템을 업그레이드합니다. host를 업그레이드하고 재부팅한 다음에는 Jail을 업그레이드할 수 있습니다. 예를 들어 12.2-RELEASE에서 12.3-RELEASE로 업그레이드를 원할경우 host에서 다음을 실행합니다:
...
여러개의 jail을 만들고 관리하는 것은 지루하고 오류가 발생하기 쉬운 작업입니다. 더크 엥글링(Dirk Engling)의 ezjail은 많은 jail 작업을 자동화하고 단순화시켜 줍니다. _basejail_ 이 템플릿으로 생성됩니다. 추가 jail은 man:mount_nullfs[8]를 사용하여 별도의 디스크 공간을 사용하지 않고 많은 basejail 디렉터리를 공유합니다. 각각의 추가 jail은 응용 프로그램이 설치되기 전에 몇 메가바이트의 디스크 공간만 차지합니다. basejail에서 사용자 공간의 복사본을 업그레이드하면 다른 모든 jail도 자동으로 업그레이드됩니다.
...
+
호스트 컴퓨터와 일치하는 FreeBSD RELEASE 기반 basejail의 경우 `install` 을 사용하십시오. 예를 들어 FreeBSD 13-STABLE로 동작하는 호스트 컴퓨터에서 FreeBSD 13의 최신 릴리스 버전이 감옥에 설치됩니다
+
[source, shell]
....
...
[TIP]
====
대부분의 네트워크 서비스는 문제 없이 jail에서 실행됩니다. 몇 가지 네트워크 서비스, 특히 man:ping[8]은 _raw 네트워크 소켓_ 을 사용합니다. jail에서 원시 네트워크 소켓은 보안을 위해 기본적으로 비활성화되어 있습니다. 이때문에 원시 네트워크 소켓이 필요한 서비스는 동작하지 않습니다.
...
이제 이 정보를 사용하여 `13.0-RELEASE` 에서 호스트 시스템의 현재 버전으로 업그레이드를 수행합니다:
