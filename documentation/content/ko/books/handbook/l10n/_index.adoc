...
_언어 코드_ 및 _국가 코드_ 는 국가 및 특정 언어 변형을 결정하는 데 사용됩니다. <<locale-lang-country>> 에서는 __언어코드_국가코드__ 의 몇 가지 예를 제공합니다:
...
`vipw` 를 사용하여 새 사용자를 추가할 때는 _language_ 를 지정하여 로캘을 설정하세요:
...
콘솔에서 여러 가지 현지화된 글꼴을 사용할 수 있습니다. 사용 가능한 글꼴 목록을 보려면 `ls /usr/share/syscons/fonts` 를 입력하세요. 콘솔 글꼴을 구성하려면 [.filename]#/etc/rc.conf# 에 [.filename]#.fnt# 접미사 없이 _폰트 이름_ 을 지정합니다:
...
사용 가능한 스크린맵 목록을 보려면 `ls /usr/share/syscons/scrnmaps` 를 입력합니다. _스크린맵 이름_ 을 지정할 때 [.filename]#.scm# 접미사를 포함하지 마세요. 화면 글꼴이 비트 8 열을 사용하는 경우, 문자가 의사 그래픽 영역을 넘어서기 때문에 VGA 어댑터의 글꼴 문자 매트릭스에서 비트 8을 비트 9로 매핑해주는 적절한 스크린 맵이 필요합니다.

사용 가능한 키맵 목록을 보려면 `ls /usr/share/syscons/keymaps` 를 입력합니다. _키맵 이름_ 을 지정할 때 [.filename]#.kbd# 접미사를 포함하지 마세요. 재부팅하지 않고 키맵을 테스트하려면 man:kbdmap[1]을 사용합니다.
...
러시아어 키보드를 활성화하려면 [.filename]#/etc/xorg.conf# 의 `”Keyboard”` 섹션에 다음을 추가하세요:
