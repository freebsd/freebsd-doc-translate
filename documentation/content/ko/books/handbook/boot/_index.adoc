...
시스템 `콘솔` 이 [.filename]#/etc/ttys# 에서 `insecure` 로 변경된 경우, 시스템은 단일 사용자 모드를 시작하기 전에 먼저 `root` 비밀번호를 묻는 메시지를 표시합니다. 이렇게 하면 보안이 강화되는 동시에 `root` 비밀번호를 알 수 없는 경우 재설정할 수 있는 기능이 제거됩니다.
...
`insecure` 콘솔은 콘솔에 대한 물리적 보안이 안전하지 않은 것으로 간주되므로 `root` 비밀번호를 아는 사람만 단일 사용자 모드를 사용할 수 있음을 의미합니다.
