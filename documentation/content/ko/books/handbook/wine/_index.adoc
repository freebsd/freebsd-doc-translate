...
* _wineconsole_ 유틸리티는 _user_ 또는 _curses_ 와 함께 사용하여 WINE 시스템이 CUI 애플리케이션에 제공하는 몇 가지 향상된 기능을 활용할 수 있습니다.
...
WINE이 무엇이고 어떻게 작동하는지에 대해 이해했다면, FreeBSD에서 효과적으로 사용하기 위한 다음 단계는 그 구성에 익숙해지는 것입니다. 다음 섹션에서는 _WINE 접두사_ 의 핵심 개념을 설명하고, 이 접두사가 WINE을 통해 실행되는 애플리케이션의 동작을 제어하는 데 어떻게 사용되는지 설명합니다.

[[wine-prefixes]]
=== 와인 접두사

와인 _접두사_ 는 디렉터리로, 일반적으로 기본 위치인 [.filename]#$HOME/.wine# 아래에 위치하지만 다른 위치에 존재할 수도 있습니다. 접두사는 와인에서 특정 애플리케이션에 필요한 Windows(R) 환경을 구성하고 실행하는 데 사용하는 구성 및 지원 파일의 집합입니다. 기본적으로 새로운 WINE 설치는 사용자가 처음 시작할 때 다음과 같은 구조를 생성합니다:
...
* [.filename]#drive_c/# : Windows(R) 시스템의 기본(즉, [.filename]#C:# ) 드라이브를 에뮬레이트합니다. 여기에는 표준 Windows(R) 시스템과 동일한 디렉토리 구조와 관련 파일이 포함됩니다. 새로운 WINE 접두사에는 _Users_ 및 _Windows_ 와 같은 Windows(R) 10 디렉터리가 포함되며, 이 디렉터리에는 OS 자체가 들어 있습니다. 또한 접두사 내에 설치된 애플리케이션은 아키텍처에 따라 _Program Files_ 또는 _Program Files (x86)_ 에 위치하게 됩니다.
...
_응용 프로그램(Applications)_ 에는 특정 버전의 Windows(R)와 프로그램을 연결할 수 있는 컨트롤이 포함되어 있습니다. 처음 시작할 때 _Application settings_ 섹션에는 단일 항목이 포함됩니다: _Default Settings_. 이는 접두사의 모든 기본 구성에 해당하며, (비활성화된 _Remove application_ 버튼에서 알 수 있듯이) 삭제할 수 없습니다.
...
. _Add_ 버튼을 클릭합니다.
. 새 오버라이드는 _Existing overrides_ 목록에 표시되며, 괄호 안에 _native, builtin_ 이라는 명칭이 표시됩니다.
. 라이브러리를 선택해 클릭하세요.
. _Edit_ 버튼을 클릭하세요.
...
* 호스트 FreeBSD 시스템의 디렉터리를 Windows(R) 환경 내의 유용한 폴더에 매핑. 기존 연결을 변경하려면 원하는 항목을 선택하고 _Browse_ 를 클릭한 다음 제공된 대화 상자를 사용하여 디렉터리를 선택합니다.
...
. _OK_ 를 클릭합니다.
. _Path_ 입력 상자에 리소스 경로를 입력하거나 _Browse_ 를 클릭하고 제공된 대화 상자를 사용하여 경로를 선택합니다.
...
_Install an application_, _Install a benchmark_ 또는 _Install a game_ 를 선택하면 아래 애플리케이션의 경우와 같이 지원되는 옵션이 포함된 목록이 표시됩니다:

image::winetricks-run-2.png[]

하나 이상의 항목을 선택하고 _OK_ 를 클릭하면 해당 설치 프로세스가 시작됩니다. 처음에는 오류로 보이는 일부 메시지가 표시될 수 있지만, 실제로는 `winetricks` 가 애플리케이션의 알려진 문제를 해결하기 위해 WINE 환경을 구성하는 과정에서 나타나는 정보 경고입니다:
...
애플리케이션을 제거하려면 `winetricks` 를 다시 실행한 후 _Run an uninstaller_ 을 선택하세요.
...
그러면 친절한 환영 메시지가 표시됩니다. 계속하려면 _OK_ 을 클릭합니다.
...
호무라가 지원하는 애플리케이션 중 하나를 선택하여 설치하려면 _Installation_ 를 선택한 후 _OK_ 를 클릭합니다. 그러면 호무라가 자동으로 설치할 수 있는 애플리케이션 목록이 표시됩니다. 하나를 선택하고 _OK_ 를 클릭하여 프로세스를 시작합니다.
...
호무라를 실행한 방법에 따라 설치가 터미널의 간단한 데스크톱 알림 또는 메시지로 끝날 수 있습니다. 그러나 어떤 경우든 호무라는 메인 화면으로 돌아와야 합니다. 설치가 성공적으로 완료되었는지 확인하려면 _Launcher_ 를 선택하고 _OK_ 를 클릭합니다.
...
새 프로그램을 실행하려면 목록에서 해당 프로그램을 선택하고 _OK_ 를 클릭합니다. 애플리케이션을 제거하려면 메인 화면에서 _Uninstallation_ 을 선택하면 비슷한 목록이 표시됩니다. 제거할 프로그램을 선택하고 _OK_ 를 클릭합니다.
...
이러한 변경의 결과로 _User_Alias_ 섹션에 지정된 사용자는 파일의 마지막 줄에 나열된 사용자처럼 _Defaults_ 섹션(현재 표시)에 나열된 리소스를 사용하여_Cmnd Alias_ 섹션에 나열된 프로그램을 실행하도록 허용됩니다. 즉, _WINDOWS_USERS_ 로 지정한 사용자는 사용자 _windows_ 로 WINE 및 `winecfg` 애플리케이션을 실행할 수 있습니다. 보너스로, 이 구성은 _windows_ 사용자의 암호를 입력할 필요가 없다는 것을 의미합니다.
...
이 섹션의 앞부분에서 설명한 것처럼 와인 패키지와 i386-wine 패키지는 서로 충돌하므로 일반적인 방법으로는 동일한 시스템에 설치할 수 없습니다. 그러나 chroots/jails과 같은 메커니즘을 사용하거나 소스에서 WINE을 빌드하여 여러 번 설치할 수 있습니다(포트를 빌드하는 것을 의미하지는 _않음_ 에 유의하세요).
